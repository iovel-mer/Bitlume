...
export const Market: React.FC = () => {
  ...
  return (
    <section className="py-20 bg-gradient-to-br from-indigo-950 via-slate-900 to-cyan-900 min-h-screen relative overflow-hidden">
      {/* Background Grid */}
      ...
      {/* Animated Orbs */}
      <div className="absolute top-1/4 left-1/6 w-96 h-96 bg-gradient-conic from-blue-400 via-indigo-500 to-purple-600 rounded-full blur-3xl opacity-30 animate-spin" style={{ animationDuration: '20s' }}></div>
      <div className="absolute bottom-1/4 right-1/6 w-80 h-80 bg-gradient-conic from-sky-400 via-cyan-500 to-indigo-500 rounded-full blur-3xl opacity-25 animate-spin" style={{ animationDuration: '25s', animationDirection: 'reverse' }}></div>
      ...
      <div className="container mx-auto px-4 md:px-6 max-w-7xl relative z-10">
        ...
        <div className="text-center mb-16">
          <div className="inline-flex items-center gap-3 px-8 py-4 bg-gradient-to-r from-cyan-500/20 to-sky-500/20 backdrop-blur-lg border-2 border-cyan-400/40 text-sm font-mono uppercase tracking-widest text-cyan-300 shadow-[0_0_30px_rgba(56,189,248,0.4)] mb-8"
          ...
          <p className="text-lg text-cyan-100 max-w-3xl mx-auto font-mono leading-relaxed">
            {t("description")}
          </p>
        </div>
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-32">
            <div className="relative">
              <div className="w-24 h-24 border-4 border-cyan-500 border-t-transparent animate-spin shadow-[0_0_30px_rgba(56,189,248,0.6)]"
                style={{ clipPath: 'polygon(20% 0%, 80% 0%, 100% 20%, 100% 80%, 80% 100%, 20% 100%, 0% 80%, 0% 20%)' }}></div>
              <RefreshCw className="w-8 h-8 text-cyan-400 absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 animate-pulse" />
            </div>
            <p className="mt-8 text-cyan-200 text-xl font-mono uppercase tracking-wider">{t("loading")}</p>
            <div className="mt-6 flex space-x-3">
              <div className="w-3 h-3 bg-blue-400 rounded-full animate-bounce shadow-[0_0_10px_rgba(56,189,248,0.8)]"></div>
              <div className="w-3 h-3 bg-cyan-400 rounded-full animate-bounce shadow-[0_0_10px_rgba(6,182,212,0.8)]" style={{ animationDelay: '0.1s' }}></div>
              <div className="w-3 h-3 bg-indigo-400 rounded-full animate-bounce shadow-[0_0_10px_rgba(99,102,241,0.8)]" style={{ animationDelay: '0.2s' }}></div>
            </div>
          </div>
        ) : error ? (
          <div className="text-center py-32">
            <div className="bg-gradient-to-br from-indigo-900/50 to-slate-900/50 backdrop-blur-xl border-2 border-purple-500/50 p-12 max-w-lg mx-auto shadow-2xl shadow-purple-500/30"
              style={{ clipPath: 'polygon(5% 0%, 95% 0%, 100% 25%, 95% 100%, 5% 100%, 0% 75%)' }}>
              <div className="w-16 h-16 bg-gradient-to-br from-purple-500 to-blue-600 flex items-center justify-center mx-auto mb-6 shadow-[0_0_20px_rgba(147,51,234,0.6)]"
                style={{ clipPath: 'polygon(20% 0%, 80% 0%, 100% 50%, 80% 100%, 20% 100%, 0% 50%)' }}>
                <TrendingDown className="w-8 h-8 text-white" />
              </div>
              <p className="text-purple-300 text-lg font-mono uppercase tracking-wide">{error}</p>
            </div>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
            {marketData.map((coin, index) => (
              <div
                key={coin.symbol}
                className="group relative w-full max-w-[350px] p-4 sm:p-6 bg-slate-900/60 hover:border-cyan-400/70 transition-all duration-500 hover:scale-105 overflow-hidden shadow-2xl hover:shadow-cyan-500/30"
                style={{
                ...
                <div className="relative z-10">
                  <div className="flex items-start justify-between mb-6">
                    <div className="flex items-center space-x-4">
                      ...
                      <div>
                        <h4 className="font-black text-lg text-cyan-100 uppercase tracking-wide pr-2 font-mono">{coin.name}</h4>
                  ...
                  <div className="mb-6">
                    ...
                    <p className="text-cyan-300 text-sm font-mono uppercase tracking-wider flex items-center gap-2">
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      <span className="text-cyan-200 font-mono text-sm uppercase tracking-wider">{t("movement")}</span>
