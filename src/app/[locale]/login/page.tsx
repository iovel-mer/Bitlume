...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-slate-900 to-gray-950 flex items-center justify-center p-4 sm:p-8 relative overflow-hidden">
      ...
      <div className="flex flex-col lg:flex-row w-full max-w-6xl rounded-2xl overflow-hidden shadow-xl bg-gradient-to-br from-gray-800/60 to-slate-900/60 backdrop-blur-xl border border-purple-400/30 relative z-10">
        {/* Left Side - Login Form */}
        <div className="flex-1 p-8 lg:p-12 flex flex-col justify-center">
          <Link href={`/${locale}`} className="inline-flex max-w-[130px] items-center text-purple-300 hover:text-white transition-colors mb-8">
            <ArrowLeft className="w-4 h-4 mr-2" />
            {t("backToHome")}
          </Link>
          <Card className="w-full bg-gray-800/50 backdrop-blur-sm border border-purple-400/30 text-white shadow-xl shadow-purple-500/10 rounded-2xl">
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    disabled={isLoading}
                    className="bg-gray-800/50 border border-purple-400/30 text-white placeholder:text-gray-500 focus:border-purple-400 focus:ring-purple-400/20 backdrop-blur-sm rounded-xl"
                    required
                  />
                </div>
                <div className="space-y-2">
                ...
                <Button
                  type="submit"
                  disabled={isLoading}
                  className="w-full cursor-pointer  bg-gradient-to-r from-purple-500 to-indigo-600 hover:from-purple-600 hover:to-indigo-700 text-white font-bold py-3 tracking-wide transition-all duration-200 transform hover:scale-105 rounded-xl shadow-lg shadow-purple-500/30"
                >
                  {isLoading ? (
       ...
       <div className="flex-1 p-8 lg:p-12 bg-gradient-to-br from-purple-900/10 to-indigo-900/10 backdrop-blur-sm flex flex-col justify-center border-l border-purple-400/30 lg:border-t-0 border-t">
          <h2 className="text-3xl font-black text-white mb-8 tracking-wide">{t("startTrading")}</h2>

          <div className="space-y-6">
            {benefits.map((benefit, index) => (
              <div key={index} className="flex items-start space-x-3">
                <div className="flex-shrink-0 w-6 h-6 bg-gradient-to-r from-purple-500 to-indigo-500 rounded-full flex items-center justify-center mt-0.5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 pt-8 mt-8 border-t border-purple-400/30">
            <div className="text-center">
              <p className="text-purple-300 text-sm mb-1 font-medium tracking-wide">{t("totalVolume")}</p>
              <p className="text-2xl font-black text-purple-400 drop-shadow-lg">$7.8B+</p>
            </div>
            <div className="text-center">
              <p className="text-purple-300 text-sm mb-1 font-medium tracking-wide">{t("activeTraders")}</p>
              <p className="text-2xl font-black text-purple-400 drop-shadow-lg">900K+</p>
            </div>
            <div className="text-center">
              <p className="text-purple-300 text-sm mb-1 font-medium tracking-wide">{t("countries")}</p>
              <p className="text-2xl font-black text-purple-400 drop-shadow-lg">195+</p>
            </div>
          </div>
        </div>
      </div>
